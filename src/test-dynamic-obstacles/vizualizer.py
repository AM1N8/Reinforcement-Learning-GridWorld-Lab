...
class GridWorldVisualizer:
    ...
    def update_features(self, goal_states: List[Tuple[int, int]], obstacles: List[Tuple[int, int]]):
        """Dynamically update grid features for a new episode."""
        self.goal_states = set(goal_states)
        self.obstacles = set(obstacles)

    def plot_grid(self, ax=None, draw_features: bool = True):
        ...
        if draw_features: # <-- Control feature drawing
            # Draw Goals
            for r, c in self.goal_states:
                # Assuming your original drawing logic for goal is here
                rect = patches.Rectangle((c, r), 1, 1, facecolor='green', edgecolor='black', alpha=0.5)
                ax.add_patch(rect)
                ax.text(c + 0.5, r + 0.5, 'G', ha='center', va='center', color='white', fontweight='bold')

            # Draw Obstacles
            for r, c in self.obstacles:
                # Assuming your original drawing logic for obstacles is here
                rect = patches.Rectangle((c, r), 1, 1, facecolor='black', edgecolor='black', alpha=0.7)
                ax.add_patch(rect)
                ax.text(c + 0.5, r + 0.5, 'X', ha='center', va='center', color='white', fontweight='bold')

            # Draw Start States
            for r, c in self.start_states:
                # Assuming your original drawing logic for start is here
                rect = patches.Rectangle((c, r), 1, 1, facecolor='blue', edgecolor='black', alpha=0.1)
                ax.add_patch(rect)
                ax.text(c + 0.5, r + 0.5, 'S', ha='center', va='center', color='blue', fontweight='bold')
    ...
    def animate_learning_process(self, episode_trajectories, episode_rewards, 
                                 ...
                                 title="Learning Process Animation", save_path=None,episode_features=None):
        """
        ...
        def init():
            ...
            return reward_line, reward_scatter
        
        def update(episode):
            if episode_features:
                features = episode_features[episode]
                self.update_features(features['goal_states'], features['obstacles'])
            
                # Save limits/aspect before clearing
                xlim = ax_grid.get_xlim()
                ylim = ax_grid.get_ylim()
            
                ax_grid.clear() # Clear the entire grid axis (including old goal/obstacles)
            
                # Redraw grid AND the new features
                self.plot_grid(ax_grid) 
            
                # Restore limits/aspect/inversion that ax_grid.clear() removes
            ...
            line, = ax_grid.plot(x_coords, y_coords, 'r-', linewidth=2, alpha=0.7)
