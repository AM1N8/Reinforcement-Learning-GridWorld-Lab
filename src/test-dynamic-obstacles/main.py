...
def parse_args():
    ...
    parser.add_argument('--dynamic-mode', action='store_true', 
                       help='Enable dynamic game mode where obstacles and goal change each episode.')
    parser.add_argument('--num-dynamic-obstacles', type=int, default=5,
                       help='Number of obstacles to randomly place in dynamic mode.')
...
def run_animated_episode(args, model, algorithm, title):
    ...
    env = GridWorldEnv(model=model, render_mode=None , dynamic_mode=args.dynamic_mode,
...
def collect_episode_data(env, algorithm, num_episodes, max_steps):
    ...
    for episode in range(num_episodes):
        ...
        episode_features.append({
            'goal_states': list(env.model.goal_states),
            'obstacles': list(env.model.obstacles)
        })
        trajectory = [state.copy()]
        total_reward = 0
    ...
    return episode_trajectories, episode_rewards , episode_features 
...
def run_dql(args, model):
    ...
    env = GridWorldEnv(model=model,dynamic_mode=args.dynamic_mode,
        num_dynamic_obstacles=args.num_dynamic_obstacles)
    ...
    if args.load_model:
        ...
        if args.animate_learning:
            episode_trajectories, episode_rewards, episode_features = collect_episode_data(env, dqn, args.episodes, args.max_steps)
        else:
    ...
    if args.animate_learning:
        # --- UPDATE UNPACKING ---
        episode_trajectories, episode_rewards, episode_features = collect_episode_data( 
